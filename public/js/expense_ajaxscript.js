...
$(document).ready(function(){
    ...
    $(document).on('click','.open_modal',function(){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#report_sort').val(data.report_sort);
                $('#publication_date').val(data.publication_date);
                $('#agency_name').val(data.agency_name);
                $('#line_number').val(data.line_number);
                $('#line_number_description').val(data.line_number_description);
                $('#fiscal_year1').val(data.fiscal_year1);
                $('#year1_forecast').val(data.year1_forecast);
                $('#year2_estimate').val(data.year2_estimate);
                $('#year3_estimate').val(data.year3_estimate);
                $('#year4_estimate').val(data.year4_estimate);
                $('#btn-save').val("update");
                $('#myModal').modal('show');
            },
            error: function (data) {
    ...
    // $("#btn-save").click(function (e) {
        ...
        e.preventDefault(); 
        var formData = {
            report_sort: $('#report_sort').val(),
            publication_date: $('#publication_date').val(),
            agency_name: $('#agency_name').val(),
            line_number: $('#line_number').val(),
            line_number_description: $('#line_number_description').val(),
            fiscal_year1: $('#fiscal_year1').val(),
            year1_forecast: $('#year1_forecast').val(),
            year2_estimate: $('#year2_estimate').val(),
            year3_estimate: $('#year3_estimate').val(),
            year4_estimate: $('#year4_estimate').val(),
        }
