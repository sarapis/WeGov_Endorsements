...
@section('content')
<style>
    .endosment_table table.dataTable tr td{
        background: #dd4b39;
        color: white;
        font-weight: 600;
    }
    ...
    <div class="demo-content mdl-color--white mdl-shadow--4dp content mdl-color-text--grey-800 mdl-cell mdl-cell--12-col">
        ...
        <div class="menu-bar row">
            <ul class="nav nav-tabs desktop_tab" role="tablist">
                ...
                <li class="active"><a href="/organization_{{$organization->organizations_id}}/indicators" id="indicators_tab">INDICATORS</a></li>
                @endif
            </ul>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="box padding_0 table_data">
                                <p class="text-desired"> <span class="badge bg-green">Desired Results: {{$desired_count}}</span>     &nbsp&nbsp&nbsp&nbsp<span class="badge bg-red">Undesired Results: {{$undesired_count}}</span></p>
                                    ...
                                    <table id="example3" class="table table-bordered" cellspacing="0" width="100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="text-align: right;">FY14</th>
                                                <th style="text-align: right;">FY15</th>
                                                <th style="text-align: right;">FY16</th>
                                                <th style="text-align: right;">FY17</th>
                                                <th style="text-align: right;">FY18</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach($indicators as $indicator)
                                            <tr>
                                                <td style="width: 50%; text-align: left;" @if($indicator->desired_direction == $indicator->trend) class="td-desired" @endif>{{$indicator->performance_indicator}}</td>
                                                <td style="text-align: right;" @if($indicator->desired_direction == $indicator->trend) class="td-desired" @endif>{{$indicator->fy14}}</td>
                                                <td style="text-align: right;" @if($indicator->desired_direction == $indicator->trend) class="td-desired" @endif>{{$indicator->fy15}}</td>
                                                <td style="text-align: right;" @if($indicator->desired_direction == $indicator->trend) class="td-desired" @endif>{{$indicator->fy16}}</td>
                                                <td style="text-align: right;" @if($indicator->desired_direction == $indicator->trend) class="td-desired" @endif>{{$indicator->fy17}}</td>
                                                <td style="text-align: right;" @if($indicator->desired_direction == $indicator->trend) class="td-desired" @endif>{{$indicator->fy18}}</td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
