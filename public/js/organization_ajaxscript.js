...
$(document).ready(function(){
    ...
    $(document).on('click','.open_modal',function(){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#organizations_id').val(data.organizations_id);
                $('#alternate_name').val(data.alternate_name);
                $('#name').val(data.name);
                $('#type').val(data.type);
                $('#website').val(data.website);
                $('#description').val(data.description);
                $('#logo').val(data.logo);
                $('#checkbook').val(data.checkbook);
                $('#internalnotes').val(data.internalnotes);
                $('#contacts_link').val(data.contacts_link);
                if(data.dedupe == 1)
    ...
    // $("#btn-save").click(function (e) {
        ...
        e.preventDefault(); 
        var formData = {
            organizations_id: $('#organizations_id').val(),
            ...
            logo: $('#logo').val(),
            checkbook:$('#checkbook').val(),
            internalnotes:$('#internalnotes').val(),
            contacts_link:$('#contacts_link').val(),
            dedupe:$('#dedupe').val(),
            publish: checked,
        }
